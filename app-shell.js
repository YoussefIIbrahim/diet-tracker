...
mobile.addEventListener('click',e=>{if(!document.body.classList.contains('dock-minimized'))return;e.preventDefault();e.stopPropagation();document.body.classList.remove('dock-minimized','scrolling-down')},true);
mobile.addEventListener('pointerenter',()=>document.body.classList.remove('dock-minimized'));
...
let lastY=scrollY;addEventListener('scroll',()=>{const y=scrollY;header.classList.toggle('is-scrolled',y>20);const down=y>lastY&&y>110;document.body.classList.toggle('scrolling-down',down);document.body.classList.toggle('dock-minimized',down&&y>140);lastY=y},{passive:true});
